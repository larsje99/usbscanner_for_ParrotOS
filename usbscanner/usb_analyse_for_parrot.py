def main():
    ...
    subprocess.run('sudo cp -r recovered_files map_to_scan', shell=True)
    ...
    subprocess.run('sudo cp -r ' + name_for_image + '.E01 drive_E01_image', shell=True)
    time.sleep(1)
    subprocess.run('sudo cp -r drive_E01_image map_to_scan', shell=True)
    
    # ZIP MAP FOR FINAL USE
    
    subprocess.run('sudo zip final_folder.zip map_to_scan/*', shell=True)
    
    #CLEAN UP
    
    subprocess.run("sudo rm -r map_to_scan/*", shell=True)
    subprocess.run("sudo rm -r recovered_files", shell=True)
    subprocess.run("sudo rm -r drive_E01_image", shell=True)
