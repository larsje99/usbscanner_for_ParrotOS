def main():
    ...
    for code in device_id:
        ...
        for line in lines:
            if line.startswith('Bus') and code in line:
                authorize_confirm = input("\nDO YOU WANT TO AUTHORIZE THIS DEVICE FOR USE (WARNING USB WILL BE ALLOWED TO THE SYSTEM): " + line + " | TYPE \'YES\' OR \'NO\' \n: ")
                if authorize_confirm == "YES":
                    subprocess.Popen(['lxterminal', '-e', f'sudo usbguard allow-device {code}'])
    ...
    # MOVE ALL LOG FILES TO LOG FOLDER
    
    if os.path.exists('log_files'):
        print("LOG DIRECTORY ALREADY EXISTS")
    else:
        subprocess.run(['sudo', 'mkdir', 'log_files'])
        
    subprocess.run('sudo mv map_to_scan/recovered_files/audit.txt log_files', shell=True)
    subprocess.run('sudo mv converted_' + name_for_image + ".raw.info log_files", shell=True)
    try:
        subprocess.run('sudo mv image_process_log log_files', shell=True)
    except:
        print("\n NO NEW IMAGE CREATED -> NO LOG FILE FOR THIS PROCESS \n")
        
    subprocess.run('sudo mv log_files map_to_scan', shell=True)
    ...
    subprocess.run("sudo rm -r converted_" + name_for_image + ".raw", shell=True)
